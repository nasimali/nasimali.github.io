...
const App: React.FC = () => {
        ...
        const observer = new IntersectionObserver(
            (entries) => {
                entries.forEach(entry => {
                    ...
                    if (entry.isIntersecting && (entry.intersectionRatio >= 0.4 || entry.boundingClientRect.top <= 150)) {
    ...
    return (
        <div className="min-h-screen bg-background text-foreground font-sans transition-colors duration-300 selection:bg-primary/70 selection:text-primary-foreground">
            ...
            />
            <main className="overflow-x-hidden pt-16"> {/* Add padding-top equal to navbar height */}
                <Hero />
                <About />
                <Skills />
                <Projects />
