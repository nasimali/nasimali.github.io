...
import { Card, CardHeader, CardTitle, CardContent, CardFooter, CardDescription } from "./ui/card.tsx";
...
const Projects: React.FC = () => {
    ...
    return (
        <section id="projects" className="py-16 md:py-24 bg-secondary/30 dark:bg-secondary/10 rounded-none sm:rounded-xl scroll-mt-16">
            <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                ...
                >
                    <h2 className="text-3xl md:text-4xl font-bold mb-4 text-primary dark:text-primary-foreground">{projectsSectionContent.heading}</h2>
                    <p className="text-lg text-muted-foreground">
                        {projectsSectionContent.subheading}
                    </p>
                </motion.div>
                                    ...
                                    <div className="aspect-video overflow-hidden">
                                        <img
                                            ...
                                            onError={(e: React.SyntheticEvent<HTMLImageElement, Event>) => {
                                                e.currentTarget.src = "https://placehold.co/600x400/CCCCCC/999999?text=Image+Error";
                                            }}
                                        />
                                    </div>
                                )}
                                <CardHeader className="pb-3 pt-5">
                                    <div className="flex items-start space-x-3">
                                        <div className="p-2.5 rounded-lg bg-primary/10 mt-0.5 group-hover:bg-primary/20 transition-colors">
                                            <DynamicIcon name={project.iconName} className={`w-7 h-7 ${project.iconColor}`} />
                                        </div>
                                        <div>
                                            <CardTitle className="text-lg sm:text-xl leading-tight">{project.title}</CardTitle>
                                            <CardDescription className="text-xs pt-0.5">{project.category}</CardDescription>
                                        </div>
                                    </div>
                                </CardHeader>
