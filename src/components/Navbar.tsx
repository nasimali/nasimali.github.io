...
const NavLink: React.FC<NavLinkProps> = ({ item, activeSection, setActiveSection, children, onClick }) => (
    <a
        href={`#${item.id}`}
...
const Navbar: React.FC<NavbarProps> = ({ darkMode, toggleDarkMode, activeSection, setActiveSection }) => {
    ...
    useEffect(() => {
        const handleResize = () => {
            if (window.innerWidth >= 768 && mobileMenuOpen) { // 768px is md breakpoint in Tailwind
